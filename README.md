...
Windows 当前支持命令行版 `ConvertVideo2MP3CLI.exe`。现有图形界面基于 macOS AppKit，不能直接编译成 Windows GUI。
...
- Windows x64 CLI 支持依赖检测、批量转换、调音、人声/背景音分离。
...
Windows x64 命令行版在 Windows PowerShell 中构建：
...
- `dist\windows-x64\ConvertVideo2MP3CLI.exe`
- `dist\ConvertVideo2MP3-windows-x64.zip`

也可以在 GitHub Actions 中手动运行 `Windows x64 CLI` 工作流生成 artifact。
